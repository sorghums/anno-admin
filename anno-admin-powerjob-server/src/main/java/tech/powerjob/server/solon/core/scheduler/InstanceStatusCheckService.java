...
import tech.powerjob.server.solon.persistence.remote.repository.*;
...
public class InstanceStatusCheckService {
    ...
    public void checkWorkflowInstance() {
        ...
        List<String> allAppIds = appInfoRepository.listAppIdByCurrentServer(transportService.defaultProtocol().getAddress());
        if (CollectionUtils.isEmpty(allAppIds)) {
            log.debug("[InstanceStatusChecker] current server has no app's job to check");
            return;
        }
        try {
    ...
    private void handleWaitingDispatchInstance(List<String> partAppIds) {
        ...
        while (!waitingDispatchInstances.isEmpty()) {
            ...
            Map<String, List<InstanceInfoDO>> waitingDispatchInstancesMap = waitingDispatchInstances.stream().collect(Collectors.groupingBy(InstanceInfoDO::getAppId));
            for (Map.Entry<String, List<InstanceInfoDO>> entry : waitingDispatchInstancesMap.entrySet()) {
                final String currentAppId = entry.getKey();
                final List<InstanceInfoDO> currentAppWaitingDispatchInstances = entry.getValue();
                // collect job id
                Set<String> jobIds = currentAppWaitingDispatchInstances.stream().map(InstanceInfoDO::getJobId).collect(Collectors.toSet());
                // query job info and map
                Map<String, JobInfoDO> jobInfoMap = jobInfoRepository.findByIdIn(jobIds).stream().collect(Collectors.toMap(JobInfoDO::getId, e -> e));
                ...
                currentAppWaitingDispatchInstances.parallelStream().forEach(instance -> {
                    ...
                    if (jobInfoOpt.isPresent()) {
                        ...
                        InstanceInfoDO instanceInfoDO = instanceInfoRepository.selectById(instance.getId());
     ...
     */
    private void checkWorkflowInstance(List<String> allAppIds) {
        ...
        Lists.partition(allAppIds, MAX_BATCH_NUM_APP).forEach(partAppIds -> {
            ...
            if (!CollectionUtils.isEmpty(waitingWfInstanceList)) {
                ...
                List<String> wfInstanceIds = waitingWfInstanceList.stream().map(WorkflowInstanceInfoDO::getWfInstanceId).collect(Collectors.toList());
