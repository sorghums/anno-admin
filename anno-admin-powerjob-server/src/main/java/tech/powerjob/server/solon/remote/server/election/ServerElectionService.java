...
public class ServerElectionService {
    ...
    @Inject("${oms.accurate.select.server.percentage}")
    ...
    private String getServer0(ServerDiscoveryRequest discoveryRequest) {
        ...
        for (int i = 0; i < RETRY_TIMES; i++) {
            ...
            // 无锁获取当前数据库中的Server
            AppInfoDO appInfoDO = appInfoRepository.selectById(appId);
            if (appInfoDO == null) {
                throw new PowerJobException(appId + " is not registered!");
            }
            String appName = appInfoDO.getAppName();
            String originServer = appInfoDO.getCurrentServer();
            ...
            try {
                ...
                if (appInfo == null) {
                    throw new PowerJobException("impossible, unless we just lost our database.");
                }
