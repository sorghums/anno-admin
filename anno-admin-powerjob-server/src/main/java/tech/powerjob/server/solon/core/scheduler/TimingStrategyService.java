...
@Slf4j
@Component
public class TimingStrategyService implements LifecycleBean {
    ...
    private Map<TimeExpressionType, TimingStrategyHandler> strategyContainer;

    @Override
    public void start() {
        // init
        strategyContainer = new EnumMap<>(TimeExpressionType.class);
        for (TimingStrategyHandler timingStrategyHandler : Solon.context().getBeansOfType(TimingStrategyHandler.class)) {
