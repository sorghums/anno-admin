...
import tech.powerjob.server.solon.common.constants.InstanceType;
...
public class InstanceService {
    ...
    /**
     * 创建任务实例（注意，该方法并不调用 insert，如果有需要立即同步到DB的需求，请在方法结束后手动调用 flush）
     * ********************************************
     ...
     * @return 任务实例ID
     */
    public InstanceInfoDO create(String jobId, String appId, String jobParams, String instanceParams, String wfInstanceId, Long expectTriggerTime) {

        String instanceId = idGenerateService.allocate();
        ...
        newInstanceInfo.setCreateTime(now);
        newInstanceInfo.setUpdateTime(now);
     ...
     */
    @DesignateServer
    public void stopInstance(String appId, String instanceId) {
        ...
        try {
            ...
            if (workerInfoOpt.isPresent()) {
                ServerStopInstanceReq req = new ServerStopInstanceReq(Long.parseLong(instanceId));
     ...
     */
    @DesignateServer
    public void retryInstance(String appId, String instanceId) {
        ...
        if (jobInfo == null) {
            throw new PowerJobException("can't find job info by jobId: " + jobId);
        }
     ...
     */
    @DesignateServer
    public void cancelInstance(String appId, String instanceId) {
    ...
    public List<InstanceInfoDTO> queryInstanceInfo(PowerQuery powerQuery) {
        log.error("queryInstanceInfo is not supported");
        return null;
    }
     ...
     * @return 任务实例的信息
     */
    public InstanceInfoDTO getInstanceInfo(String instanceId) {
     ...
     * @return 详细运行状态
     */
    public InstanceDetail getInstanceDetail(String instanceId) {
        ...
        if (instanceStatus != RUNNING) {
            BeanUtil.copyProperties(instanceInfoDO, detail);
            return detail;
        }
        ...
        if (workerInfoOpt.isPresent()) {
            WorkerInfo workerInfo = workerInfoOpt.get();
            ServerQueryInstanceStatusReq req = new ServerQueryInstanceStatusReq(Long.parseLong(instanceId));
            try {
    ...
    private InstanceInfoDO fetchInstanceInfo(String instanceId) {
    ...
    private static InstanceInfoDTO directConvert(InstanceInfoDO instanceInfoDO) {
        ...
        BeanUtil.copyProperties(instanceInfoDO, instanceInfoDTO);
