...
public class BaseOrgMetaModel extends BaseMetaModel implements Serializable {
    ...
    @AnnoField(title = "组织ID", tableFieldName = "org_id", search = @AnnoSearch, edit = @AnnoEdit(), show = false, dataType = AnnoDataType.OPTIONS, optionType = @AnnoOptionType(sql = "select id as  value, org_name as label from sys_org where del_flag = 0 order by id desc"))
